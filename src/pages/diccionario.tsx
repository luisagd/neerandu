...
function Diccionario() {
	...
	if (query !== "" && query !== "/" && query !== null) {
		if (wordData) {
			if (query !== wordData.word) {
				fetchData(query);
			}
		} else {
			console.log("URL word: " + query);
			fetchData(query);
		}
	}
	const updateQuery = () => {
		console.log("RUNNING FROM INSIDE THE UPDATING FUNCTION");
		const query = getQuery();
		if (query !== "" && query !== "/" && query !== null) {
			if (wordData) {
				console.log("URL word from inside the query: " + query);
				if (query !== wordData.word) {
					fetchData(query);
				}
			} else {
	...
	useEffect(() => {}, []); // 👈️ empty dependencies array
	return (
		<div>
